...
export default function InstituteDetail() {
  ...
  return (
    <motion.div 
      ...
      className="max-w-6xl mx-auto p-4 md:p-6 lg:p-8 space-y-8 md:space-y-10 pb-20"
    >
      ...
      <div className="bg-white rounded-xl shadow-sm hover:shadow-md border border-gray-100 relative overflow-hidden transition-all duration-300">
        ...
        <div className="px-6 md:px-10 pb-6 md:pb-10 pt-4 flex flex-col md:flex-row gap-6 md:gap-8 items-start relative">
          <div className="-mt-16 sm:-mt-20 md:-mt-24 relative z-10 p-1.5 bg-white rounded-[22px] md:rounded-[30px] shadow-sm flex-shrink-0">
             {institute.logo ? (
          ...
          <div className="flex-1 z-10 pt-2 min-w-0 w-full">
            <h1 className="text-2xl sm:text-3xl md:text-[40px] font-bold text-slate-900 tracking-tight leading-tight break-words capitalize">{institute.name}</h1>
            <div className="flex flex-col md:flex-row md:flex-wrap gap-3 md:gap-6 mt-3 md:mt-4 text-sm md:text-base font-medium text-slate-500 w-full">
              {institute.address && <div className="flex items-center gap-2"><MapPin className="w-4 h-4 opacity-70 shrink-0"/> <span className="truncate capitalize">{institute.address}</span></div>}
              {institute.phone && <div className="flex items-center gap-2"><Phone className="w-4 h-4 opacity-70 shrink-0"/> <span className="truncate">{institute.phone}</span></div>}
              {institute.email && <div className="flex items-center gap-2"><Mail className="w-4 h-4 opacity-70 shrink-0"/> <span className="truncate">{institute.email}</span></div>}
              {institute.website && <div className="flex items-center gap-2"><Globe className="w-4 h-4 opacity-70 shrink-0"/> <a href={institute.website} target="_blank" className="text-blue-600 hover:text-blue-700 hover:underline transition-colors truncate">Website</a></div>}
            </div>
          </div>
        </div>
      </div>
                ...
                >
                  <div className="flex items-center justify-between mb-5 flex-col sm:flex-row gap-4">
                    <div className="min-w-0 flex-1 w-full text-left">
                      <h3 className="text-xl font-semibold text-slate-800 tracking-tight break-words capitalize">{batch.batch_name}</h3>
                      <div className="mt-2">
                    ...
                    <div className="flex items-center gap-3 bg-slate-50 pl-2 pr-4 py-2 rounded-full border border-slate-200 shadow-sm transition-all duration-300 hover:shadow-md sm:shrink-0 w-full sm:w-auto self-start sm:self-auto">
                      <div className="w-8 h-8 md:w-10 md:h-10 rounded-full overflow-hidden bg-indigo-100 border border-slate-200 flex items-center justify-center shrink-0">
                        {batch.teacher_image ? <img src={batch.teacher_image} className="w-full h-full object-cover"/> : <div className="w-full h-full flex items-center justify-center font-bold text-xs text-indigo-700 capitalize">{batch.teacher_name?.charAt(0) || '?'}</div>}
                      </div>
                      <span className="text-[14px] font-semibold text-slate-700 break-words line-clamp-1 capitalize">{batch.teacher_name}</span>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-x-5 gap-y-6 pt-6 border-t border-slate-200">
                    <div className="flex flex-col gap-1 text-[13px] md:text-[14px]">
                      ...
                      <span className="text-xs md:text-sm font-semibold text-slate-800 capitalize">{batch.start_date || 'TBA'}</span>
                    </div>
                    <div className="flex flex-col gap-1 text-[13px] md:text-[14px]">
                    ...
                    <div className="flex flex-col gap-1 text-[13px] md:text-[14px]">
                      ...
                      <span className="text-xs md:text-sm font-semibold text-slate-800 capitalize">{batch.batch_duration || '-'}</span>
                    </div>
                    <div className="flex flex-col gap-1 text-[13px] md:text-[14px]">
                      ...
                      <span className="text-xs md:text-sm font-semibold text-slate-800 capitalize">{batch.fee_structure || 'Contact for fee'}</span>
                    </div>
                  </div>
                </motion.div>
              )) : (
